...
exports.createConstant = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!constants) {
    return next(new ErrorHandler("Cannot create constant. Please try later.", 400));
  }
...
exports.getAllBanners = catchAsyncErrors(async (req, res, next) => {
    const banners = await Constants.findOne({ name: CONSTANTS.CONSTANTS.BANNERS });
...
exports.updateBanners = catchAsyncErrors(async (req, res, next) => {
  ...
  let constants = await Constants.findOneAndUpdate({ name: CONSTANTS.CONSTANTS.BANNERS }, {metadata: {"banners": imageLinks}}, {
    new: true,
    runValidators: true,
